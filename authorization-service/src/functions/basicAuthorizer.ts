...
export const basicAuthorizer = async (event: APIGatewayTokenAuthorizerEvent, context, cb: Callback<APIGatewayAuthorizerResult>) => {
  if (event.type !== 'TOKEN') {
    cb('Unauthorized');
  }
  ...
  try {
    ...
    const policy = generatePolicy(credentials, effect, methodArn);
